...
export const ExportDialog: React.FC<ExportDialogProps> = ({ isOpen, projectId, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[310] flex items-center justify-center bg-[#172033]/25 p-5 backdrop-blur-sm">
      <div className="w-full max-w-lg overflow-hidden rounded-2xl border border-[#dce1e7] bg-white p-6 text-[#172033] shadow-[0_28px_80px_rgba(23,32,51,0.2)]">
        ...
        <div className="mt-6 space-y-3">
          {options.map((option) => (
            <button
              ...
              className={`flex w-full gap-4 rounded-xl border p-4 text-left transition ${type === option.value ? 'border-[#ff9a78] bg-[#fff2ed]' : 'border-[#dce1e7] bg-[#fafbfc] hover:bg-white'}`}
            >
              <span className={type === option.value ? 'text-[#ff6b3d]' : 'text-[#667085]'}>{option.icon}</span>
              <span>
        ...
        <div className="mt-6 flex justify-end gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-md bg-[#172033] px-5 py-2.5 text-sm font-semibold text-white shadow-[3px_3px_0_#ff6b3d] disabled:opacity-40"
          >
