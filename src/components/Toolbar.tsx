...
export const Toolbar: React.FC<ToolbarProps> = ({
  ...
  return (
    <div className={`fixed left-4 top-1/2 -translate-y-1/2 flex flex-col items-center gap-2 p-1 rounded-xl shadow-[0_16px_40px_rgba(17,17,17,0.12)] z-50 transition-colors duration-300 ${isDark ? 'bg-[#1a1a1a] border border-neutral-800' : 'bg-white border border-[#d9d9d9]'
      }`}>
      <button
        className={`w-10 h-10 rounded-lg flex items-center justify-center hover:-translate-y-0.5 transition-all duration-200 mb-2 ${isDark ? 'bg-white text-black hover:bg-neutral-200' : 'bg-[#111111] text-white hover:bg-[#333333] shadow-[3px_3px_0_#777777]'
          }`}
        onClick={onAddClick}
      >
        <Plus size={20} />
          ...
          {isToolsOpen && (
            <div className={`absolute left-10 top-0 rounded-lg shadow-2xl py-2 min-w-[240px] z-50 ${isDark ? 'bg-[#1a1a1a] border border-neutral-700' : 'bg-white border border-neutral-200'
              ...
              >
                ...
                <div className="text-left">
                  <p className={`text-sm ${isDark ? 'text-neutral-200 group-hover:text-white' : 'text-neutral-700 group-hover:text-neutral-900'}`}>AI 分镜生成器</p>
                  <p className={`text-xs ${isDark ? 'text-neutral-500' : 'text-neutral-400'}`}>使用 AI 创建镜头</p>
                </div>
              </button>
            </div>
          )}
      ...
      <button className={`studio-utility mb-2 flex h-8 w-8 items-center justify-center rounded-md text-[8px] font-bold transition-all duration-200 hover:-translate-y-0.5 ${isDark ? 'border border-neutral-700 text-neutral-300' : 'border border-[#d9d9d9] bg-[#f3f3f3] text-[#111111]'
        }`}>
        DEMO
      </button>
    </div>
